...
operation. The diagnostic view snapshots entry keys under the GRD entry table
scan lock and then re-looks up each entry through the normal pin/release API
before taking the per-entry snapshot, so cold reclaim can safely remove
holderless entries while diagnostics are active.
