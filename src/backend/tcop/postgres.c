...
valgrind_report_error_query(const char *query)
{
	...
	if (unlikely(valgrind_error_count != old_valgrind_error_count) && query != NULL)
		VALGRIND_PRINTF("Valgrind detected %u error(s) during execution of \"%s\"\n",
						valgrind_error_count - old_valgrind_error_count, query);
}
 ...
 */
List *
pg_analyze_and_rewrite_varparams(RawStmt *parsetree, const char *query_string, Oid **paramTypes,
 ...
 */
List *
pg_analyze_and_rewrite_withcb(RawStmt *parsetree, const char *query_string,
...
exec_simple_query(const char *query_string)
{
	...
	foreach (parsetree_item, parsetree_list) {
		...
		if (lnext(parsetree_list, parsetree_item) != NULL) {
			per_parsetree_context = AllocSetContextCreate(
				MessageContext, "per-parsetree message context", ALLOCSET_DEFAULT_SIZES);
		 ...
		 */
		(void)PortalRun(portal, FETCH_ALL, true, /* always top level */
						true,					 /* ignored */
	...
	case 1:
		...
		break;
	case 2:
		ereport(LOG, (errmsg("duration: %s ms  statement: %s", msec_str, query_string),
				   ...
				   int numParams)			 /* number of parameters */
{
	...
	ereport(DEBUG2,
			(errmsg_internal("parse %s: %s", *stmt_name ? stmt_name : "<unnamed>", query_string)));
	...
	if (parsetree_list != NIL) {
		 ...
		 */
		querytree_list = pg_analyze_and_rewrite_varparams(raw_parse_tree, query_string, &paramTypes,
														  &numParams, NULL);
	...
	CompleteCachedPlan(psrc, querytree_list, unnamed_stmt_context, paramTypes, numParams, NULL,
					   NULL, CURSOR_OPT_PARALLEL_OK, /* allow parallel mode */
					   true);						 /* fixed result */
...
exec_bind_message(StringInfo input_message)
{
	...
	ereport(DEBUG2, (errmsg_internal("bind %s to %s", *portal_name ? portal_name : "<unnamed>",
	...
	if (numParams > 0) {
					...
					if (log_parameter_max_length_on_error != 0) {
						...
						if (log_parameter_max_length_on_error < 0)
							...
							knownTextValues[paramno]
								= pnstrdup(pstring, log_parameter_max_length_on_error
		...
		if (log_parameter_max_length_on_error != 0)
			params->paramValuesStr
				= BuildParamLogString(params, knownTextValues, log_parameter_max_length_on_error);
	} else
		params = NULL;
	 ...
	 */
	PortalDefineQuery(portal, saved_stmt_name, query_string, psrc->commandTag, cplan->stmt_list,
					  cplan);
...
exec_execute_message(const char *portal_name, long max_rows)
{
	...
	if (check_log_statement(portal->stmts)) {
		ereport(LOG,
				(errmsg("%s %s%s%s: %s", execute_is_fetch ? _("execute fetch from") : _("execute"),
						prepStmtName, *portal_name ? "/" : "", *portal_name ? portal_name : "",
						sourceText),
	...
	case 2:
		ereport(LOG,
				(errmsg("duration: %s ms  %s %s%s%s: %s", msec_str,
						...
						*portal_name ? "/" : "", *portal_name ? portal_name : "", sourceText),
				 errhidestmt(true), errdetail_params(portalParams)));
		break;
	}
...
{
	if (log_duration || log_min_duration_sample >= 0 || log_min_duration_statement >= 0
		|| xact_is_sampled) {
		long secs;
		int usecs;
		int msecs;
		...
		TimestampDifference(GetCurrentStatementStartTimestamp(), GetCurrentTimestamp(), &secs,
							&usecs);
		msecs = usecs / 1000;
...
bind_param_error_callback(void *arg)
{
	...
	if (data->paramval) {
		initStringInfo(&buf);
		appendStringInfoStringQuoted(&buf, data->paramval, log_parameter_max_length_on_error);
		quotedval = buf.data;
	} else
		quotedval = NULL;
	...
	if (data->portalName && data->portalName[0] != '\0') {
		if (quotedval)
			errcontext("portal \"%s\" parameter $%d = %s", data->portalName, data->paramno + 1,
					   quotedval);
		else
			errcontext("portal \"%s\" parameter $%d", data->portalName, data->paramno + 1);
	} else {
		if (quotedval)
			errcontext("unnamed portal parameter $%d = %s", data->paramno + 1, quotedval);
		else
			errcontext("unnamed portal parameter $%d", data->paramno + 1);
	}
...
start_xact_command(void)
{
	...
	if (client_connection_check_interval > 0 && IsUnderPostmaster && MyProcPort
		&& !get_timeout_active(CLIENT_CONNECTION_CHECK_TIMEOUT))
		enable_timeout_after(CLIENT_CONNECTION_CHECK_TIMEOUT, client_connection_check_interval);
}
...
ProcessInterrupts(void)
{
	...
	if (ProcDiePending) {
		...
		if (ClientAuthInProgress)
			...
			ereport(DEBUG1, (errmsg_internal("logical replication launcher shutting down")));
...
#ifdef USE_PGRAC_CLUSTER
	/*
	 * PGRAC: spec-2.28 Sprint A Step 2 D4 — fence-lite in-flight abort.
	 *
	 *	Per spec-2.28 §3.7 C4 (read-clear-then-decide), enforced inside
	 *	cluster_fence_check_interrupts:
	 *
	 *	  if (ClusterFenceFreezePending == 0) return;
	 *	  ClusterFenceFreezePending = 0;        // clear FIRST
	 *	  if (!cluster_freeze_writes_enabled || !IsTransactionState())
	 *	      return;                           // disabled / idle absorb
	 *	  ereport(ERROR, ERRCODE_CLUSTER_QUORUM_LOST_BACKEND, ...);
	 *
	 *	Per spec-2.28 §3.6 v0.3 F1 amend, no manual CritSection guard
	 *	needed:  PG ProcessInterrupts() already returned early at
	 *	postgres.c:3226-3227 if CritSectionCount > 0, so the hook
	 *	is unreachable inside a CS.
	 *
	 *	Hook anchor:  AFTER ProcDiePending block (lines 3231-3289 above)
	 *	+ BEFORE CheckClientConnectionPending (below) — once a process
	 *	exit is pending we want to die first, but in-flight quorum-loss
	...
	if (IdleInTransactionSessionTimeoutPending) {
		...
		if (IdleInTransactionSessionTimeout > 0)
			ereport(FATAL, (errcode(ERRCODE_IDLE_IN_TRANSACTION_SESSION_TIMEOUT),
	...
	if (IdleSessionTimeoutPending) {
		...
		if (IdleSessionTimeout > 0)
			ereport(FATAL, (errcode(ERRCODE_IDLE_SESSION_TIMEOUT),
	 ...
	 */
	if (IdleStatsUpdateTimeoutPending && DoingCommandRead && !IsTransactionOrTransactionBlock()) {
...
check_stack_depth(void)
{
	if (stack_is_too_deep()) {
		ereport(
			ERROR,
			(errcode(ERRCODE_STATEMENT_TOO_COMPLEX), errmsg("stack depth limit exceeded"),
...
process_postgres_switches(int argc, char *argv[], GucContext ctx, const char **dbname)
{
	...
	if (errs || argc != optind) {
		...
		if (IsUnderPostmaster)
			...
			ereport(FATAL, errcode(ERRCODE_SYNTAX_ERROR),
					errmsg("%s: invalid command-line argument: %s", progname, argv[optind]),
					errhint("Try \"%s --help\" for more information.", progname));
	}
...
PostgresMain(const char *dbname, const char *username)
{
	 ...
	 */
	MessageContext
		= AllocSetContextCreate(TopMemoryContext, "MessageContext", ALLOCSET_DEFAULT_SIZES);
	...
	row_description_context
		= AllocSetContextCreate(TopMemoryContext, "RowDescriptionContext", ALLOCSET_DEFAULT_SIZES);
...
ShowUsage(const char *title)
{
	...
	appendStringInfo(&str, "!\t[%ld.%06ld s user, %ld.%06ld s system total]\n", (long)user.tv_sec,
					 (long)user.tv_usec, (long)sys.tv_sec, (long)sys.tv_usec);
#ifndef WIN32
	...
	appendStringInfo(&str, "!\t%ld/%ld [%ld/%ld] page faults/reclaims, %ld [%ld] swaps\n",
					 r.ru_majflt - Save_r.ru_majflt, r.ru_minflt - Save_r.ru_minflt, r.ru_majflt,
					 r.ru_minflt, r.ru_nswap - Save_r.ru_nswap, r.ru_nswap);
	appendStringInfo(&str, "!\t%ld [%ld] signals rcvd, %ld/%ld [%ld/%ld] messages rcvd/sent\n",
					 r.ru_nsignals - Save_r.ru_nsignals, r.ru_nsignals,
					 r.ru_msgrcv - Save_r.ru_msgrcv, r.ru_msgsnd - Save_r.ru_msgsnd, r.ru_msgrcv,
					 r.ru_msgsnd);
...
log_disconnections(int code, Datum arg)
{
	...
	ereport(LOG,
			(errmsg("disconnection: session time: %d:%02d:%02d.%03d "
					...
					hours, minutes, seconds, msecs, port->user_name, port->database_name,
