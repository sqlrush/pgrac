...
static bool
rdma_inbound_read(int32 *out_sender_node_id, void *buf, size_t bufsize,
...
static bool
rdma_peer_add_pending_release(ClusterICRdmaPeer *peer, ClusterICSgeReleaseCallback cb,
							  void *arg)
{
	int i;
...
rdma_peer_ensure_buffers(ClusterICRdmaPeer *peer)
{
	...
	peer->recv_mr = ibv_reg_mr(RdmaCtx.pd, peer->recv_buf, peer->recv_buf_len,
							   IBV_ACCESS_LOCAL_WRITE);
	if (peer->recv_mr == NULL) {
...
rdma_prepare_peer_for_connect(int32 peer_id, struct rdma_cm_id *id, bool active_side)
{
	...
	if (!rdma_create_cm_qp(peer) || !rdma_peer_ensure_buffers(peer)
		|| !rdma_post_peer_recv(peer)) {
		cluster_ic_rdma_stats_note_error(peer_id, "58R16",
										 RdmaUnavailableReason != NULL
											 ? RdmaUnavailableReason
											 : "RDMA peer preparation failed");
		return false;
	}
...
rdma_process_recv_completion(ClusterICRdmaPeer *peer, uint32 byte_len)
{
	...
	if (!rdma_post_peer_recv(peer))
		rdma_peer_fail_or_fallback(peer->peer_id,
...
rdma_dispatch_pending_frames(void)
{
	for (;;) {
		...
		vrc = cluster_ic_envelope_verify(&env, payload, env.payload_length,
										 (uint32)cluster_node_id, sender);
		if (vrc == CLUSTER_IC_ENVELOPE_OK) {
			...
			cluster_ic_rdma_stats_note_error(sender, "53R20", "RDMA envelope dropped by epoch guard");
		} else {
				   ...
				   const ClusterICSge *payload_sge, int n_sge, uint32 payload_len)
{
	...
	rc = RdmaProvider->post_send(&peer->qp, send_sge, send_sge_count, true, 0);
...
cluster_ic_rdma_shmem_init(void)
{
	...
	RdmaShmem
		= (ClusterICRdmaShmem *)ShmemInitStruct("pgrac cluster_ic_rdma",
...
rdma_send_envelope_sge_fallback(const ClusterICEnvelope *env, int32 dest_node_id,
								const ClusterICSge *payload_sge, int n_sge,
								uint32 payload_len)
{
	char *frame = NULL;
	char *cursor;
								  ...
								  uint32 payload_len)
{
	...
	if (msg_type == 0 || (int)msg_type >= CLUSTER_IC_MSG_TYPE_MAX)
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						errmsg("cluster_ic_rdma_send_envelope_sge: msg_type %u out of range",
							   msg_type)));
	...
	if (!sum_ok || summed_len != payload_len)
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						...
						errdetail("n_sge=%d, summed_len=%u, payload_len=%u",
								  n_sge, summed_len, payload_len)));

	if (!cluster_ic_envelope_build(&env, msg_type, (uint32)cluster_node_id,
	...
	if (!cluster_ic_rdma_block_sge_supported(&reason)
		...
		cluster_ic_rdma_stats_note_fallback(dest_node_id,
											reason != NULL ? reason
														   : "peer selected TCP fallback");
		return rdma_send_envelope_sge_fallback(&env, dest_node_id, payload_sge, n_sge,
											   payload_len);
	}
...
#if defined(HAVE_LIBIBVERBS) && defined(HAVE_LIBRDMACM) && defined(HAVE_RDMA_RDMA_CMA_H)
	{
		...
		if (rdma_rc == CLUSTER_IC_SEND_WOULD_BLOCK)
			cluster_ic_rdma_stats_note_fallback(dest_node_id,
												"RDMA SEND-with-SGE backpressured; using TCP fallback");
		else
			cluster_ic_rdma_stats_note_fallback(dest_node_id,
...
rdma_lmon_report_start_failure(const char *reason)
{
	...
	ereport(LOG,
			(errmsg("RDMA CM listener setup failed; TCP fallback remains active"),
			 errdetail("%s", reason != NULL ? reason : "unknown RDMA CM failure")));
}
#endif
...
cluster_ic_rdma_lmon_handle_cm_events(void)
{
	...
	case RDMA_CM_EVENT_CONNECT_REQUEST:
		if (!rdma_verify_private_hello(private_data, private_data_len, -1, &peer_id,
									   &reason)) {
		...
		if (!rdma_prepare_peer_for_connect(peer_id, id, false)) {
			rdma_ack_cm_event(event);
			rdma_peer_fail_or_fallback(peer_id,
									   RdmaUnavailableReason != NULL ? RdmaUnavailableReason
...
static bool
rdma_provider_reg_region(ClusterICRdmaCtx *ctx,
						 void *base,
						 size_t len,
						 ClusterICMr *out)
{
...
rdma_provider_qp_create(ClusterICRdmaCtx *ctx, int32 peer, ClusterICQp *out)
{
	...
	if (ctx == NULL || out == NULL || ctx->pd == NULL || ctx->cq == NULL || !rdma_valid_peer_id(peer)) {
...
static ClusterICSendResult
rdma_provider_post_send(ClusterICQp *qp,
						const ClusterICSge *sge,
						int n_sge,
						bool signaled,
...
cluster_get_ic_rdma_peers(PG_FUNCTION_ARGS)
{
...

	{
		...
		for (i = 0; i < ClusterConfShmem->node_count; i++) {
			...
			values[16] = Int64GetDatum(p != NULL ? (int64)pg_atomic_read_u64(&p->latency_us_sum)
												  : 0);
