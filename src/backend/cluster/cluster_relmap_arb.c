	...
	if (cluster_peer_mode_enabled())
	{
		...
		for (attempt = 0; attempt < CLUSTER_RELMAP_ARB_BCAST_RETRIES; attempt++)
		{
			...
			if (r == CLUSTER_SINVAL_ACK_DONE)
				break;
			ereport(WARNING,
					(errmsg("cluster relmap arbitration invalidation not "
							"acknowledged (attempt %d of %d), retrying",
							attempt + 1, CLUSTER_RELMAP_ARB_BCAST_RETRIES)));
		}
		if (r != CLUSTER_SINVAL_ACK_DONE)
			ereport(PANIC,
					(errmsg("cluster relmap arbitration invalidation was not "
...
arbitrate_one(bool shared_map, Oid dbid)
{
	...
	else
	{
		...
		switch (cluster_remote_outcome_durable_checked((int) hdr.owner_node,
														hdr.owner_xid, &scn))
		{
	...
	{
		ereport(LOG,
				(errmsg("cluster relmap pending image for %s (generation %llu, "
	...
	if (committed)
	{
		...
		ereport(LOG,
				(errmsg("cluster relmap arbitration published pending generation "
	...
	{
		cluster_relmap_authority_discard_pending(shared_map, dbid,
												 hdr.pending_generation);
		ereport(LOG,
				(errmsg("cluster relmap arbitration discarded pending generation "
