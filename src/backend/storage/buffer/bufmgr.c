...
typedef struct ClusterBufmgrGcsBorrow {
	BufferDesc *buf;
	LWLock	   *content_lock;
} ClusterBufmgrGcsBorrow;
...
/*
 * Borrow the stable shared_buffers page for RDMA SEND-with-SGE.
 *
 * The lookup/WAL-flush/revalidation contract is identical to
 * cluster_bufmgr_copy_block_for_gcs().  On success, the returned page remains
 * protected by a raw buffer pin and content_lock SHARED until
 * cluster_bufmgr_release_borrowed_block_for_gcs() is called.
 */
bool
cluster_bufmgr_borrow_block_for_gcs(BufferTag tag, XLogRecPtr *out_page_lsn,
									const char **out_block, void **out_release_arg)
{
	...
	int			retries;
	Page		page;
	ClusterBufmgrGcsBorrow *borrow;

	if (out_page_lsn == NULL || out_block == NULL || out_release_arg == NULL)
		return false;

	Assert(out_page_lsn != NULL);
	Assert(out_block != NULL);
	Assert(out_release_arg != NULL);

	*out_block = NULL;
	*out_release_arg = NULL;

	borrow = (ClusterBufmgrGcsBorrow *)
		MemoryContextAlloc(TopMemoryContext, sizeof(ClusterBufmgrGcsBorrow));
	...
	for (retries = 0; retries < 2; retries++)
	{
		...
		{
			borrow->buf = buf;
			borrow->content_lock = content_lock;
			*out_page_lsn = second_lsn;
			*out_block = (const char *) page;
			*out_release_arg = borrow;
			return true;
		}
		LWLockRelease(content_lock);
	}
...
void
cluster_bufmgr_release_borrowed_block_for_gcs(void *release_arg)
{
	ClusterBufmgrGcsBorrow *borrow = (ClusterBufmgrGcsBorrow *) release_arg;

	if (borrow == NULL)
		return;

	if (borrow->content_lock != NULL)
		LWLockRelease(borrow->content_lock);
	if (borrow->buf != NULL)
		cluster_bufmgr_unpin_for_gcs(borrow->buf);
	pfree(borrow);
}
