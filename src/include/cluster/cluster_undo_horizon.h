...
/*
 * Retired idle-unconstrained report value. A finite future snapshot cannot
 * be bounded by earlier instantaneous idleness. The old consumer self-fence
 * correctly refused such snapshots but made normal idle-to-active traffic
 * fail. New senders publish only finite bounds; handler and fold reject this
 * value. Keep the numeric constant for identifying old wire/artifact input.
...
/* Historical idle-sentinel send counter; finite-only senders leave it zero. */
