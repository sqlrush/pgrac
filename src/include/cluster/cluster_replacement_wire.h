...
#define CANDIDATE2_CORRECTED_A1_GRAMMAR_FINGERPRINT UINT64_C(0x8e0dae5b428905e4)
...
typedef struct ClusterReplacementPhase3Handoff {
	uint64 producer_seq;
	uint64 consumer_seq;
	ClusterReplacementPhase3HandoffItem items[CLUSTER_REPLACEMENT_PHASE3_HANDOFF_CAPACITY];
...
extern bool cluster_replacement_wire_encode(const ClusterReplacementWireMessage *message,
...
cluster_replacement_wire_encode_phase3_snapshot(const ClusterR4PrerequisiteSnapshot *snapshot,
												uint8 out[CLUSTER_REPLACEMENT_WIRE_BYTES]);
extern bool cluster_replacement_wire_decode(const uint8 bytes[CLUSTER_REPLACEMENT_WIRE_BYTES],
											ClusterReplacementWireMessage *out);
extern void cluster_replacement_phase3_handoff_init(ClusterReplacementPhase3Handoff *handoff);
extern uint32
cluster_replacement_phase3_handoff_pending(const ClusterReplacementPhase3Handoff *handoff);
extern bool cluster_replacement_phase3_handoff_poll(ClusterReplacementPhase3Handoff *handoff,
													ClusterReplacementPhase3HandoffItem *out);
extern ClusterReplacementPhase3IngressResult cluster_replacement_wire_phase3_ingress(
	ClusterReplacementPhase3Handoff *handoff, const ClusterICEnvelope *env, const void *payload,
	uint32 payload_length, int32 authenticated_source_node_id, int32 local_receiver_node_id,
...
extern ClusterReplacementPhase3IngressResult cluster_replacement_wire_phase3_ingress_local(
	...
	int32 authenticated_source_node_id, int32 local_receiver_node_id, uint64 current_epoch,
	uint32 control_connection_generation);
extern bool cluster_replacement_phase3_handoff_poll_local(ClusterReplacementPhase3HandoffItem *out);
