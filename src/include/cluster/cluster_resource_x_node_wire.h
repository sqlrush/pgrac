...
StaticAssertDecl(offsetof(ResourceXLocalProofDeclarationV1, local_holder_authority_generation) == 96
					 && offsetof(ResourceXLocalProofDeclarationV1, local_proof_generation) == 144,
...
StaticAssertDecl(offsetof(ResourceXInstallSettlementV1, conversion_base_generation) == 96
					 && offsetof(ResourceXInstallSettlementV1, settlement_flags) == 148,
				 "Resource-X install-settlement V1 offsets changed");

extern bool cluster_resource_x_wire_encode(uint8 msg_type, const ResourceXDecodedFrame *frame,
										   void *payload, uint16 payload_capacity,
										   uint16 *payload_len_out, ResourceXWireReject *reject);
extern bool cluster_resource_x_wire_decode(uint8 msg_type, const void *payload, uint16 payload_len,
										   ResourceXDecodedFrame *out, ResourceXWireReject *reject);
extern bool cluster_resource_x_wire_rebind_sender_generation(uint8 msg_type, void *payload,
															 uint16 payload_len,
															 uint32 sender_connection_generation,
