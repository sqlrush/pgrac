...
bool cluster_cf_phase2_peer_verified(void) { return false; }
...
ClusterCssdStatus cluster_cssd_get_status(void) { return g_cssd_status; }
int cluster_cssd_get_alive_peer_count(void) { return g_alive_peers; }
bool cluster_qvotec_in_quorum(void) { return g_in_quorum; }

/* ---- stubs for the bootstrap-window orchestrator's deps (not exercised) ---- */
bool		cluster_enabled = false;
int cluster_conf_node_count(void) { return 1; }
void cluster_cf_set_bootstrap_authority(bool on) { (void) on; }
void cluster_cf_set_join_readonly(bool on) { (void) on; }
void cluster_cf_set_write_skip(bool on) { (void) on; }
...
bool cluster_cf_bak_checkpoint_recoverable(const ControlFileData *bak pg_attribute_unused()) { return true; }
...
void cluster_cf_counter_inc(ClusterCfCounter which pg_attribute_unused()) {}
...
UT_TEST(test_symlink_status)
{
	...
	/* missing */
	UT_ASSERT_EQ(cluster_cf_symlink_status(local, target),
...
UT_TEST(test_migrate_and_link)
{
	...
	UT_ASSERT(cluster_cf_migrate_and_link(pgdata));
	UT_ASSERT_EQ(cluster_cf_symlink_status(local, cluster_cf_shared_path()),
				 CLUSTER_CF_SYMLINK_OK);
}
...
UT_TEST(test_contract_resolve)
{
	...
	UT_ASSERT_EQ(cluster_cf_contract_resolve("aaa", "aaa",
	...
	UT_ASSERT_EQ(cluster_cf_contract_resolve("aaa", "bbb",
											 CLUSTER_CF_CONTRACT_CROSSNODE_VERIFIED),
				 CLUSTER_CF_CONTRACT_UNVERIFIED);
	/* no current identity -> cannot bind -> UNVERIFIED */
	UT_ASSERT_EQ(cluster_cf_contract_resolve("aaa", "",
											 CLUSTER_CF_CONTRACT_CROSSNODE_VERIFIED),
				 CLUSTER_CF_CONTRACT_UNVERIFIED);
	UT_ASSERT_EQ(cluster_cf_contract_resolve("aaa", NULL,
											 CLUSTER_CF_CONTRACT_CROSSNODE_VERIFIED),
				 CLUSTER_CF_CONTRACT_UNVERIFIED);
...
UT_TEST(test_contract_persist_load)
{
	...
	strlcpy(g_storage_uuid, "00112233445566778899aabbccddeeff",
			sizeof(g_storage_uuid));
	...
	UT_ASSERT(cluster_cf_contract_persist(pgdata,
										  CLUSTER_CF_CONTRACT_CROSSNODE_VERIFIED));
	UT_ASSERT_EQ(cluster_cf_contract_load(pgdata),
				 CLUSTER_CF_CONTRACT_CROSSNODE_VERIFIED);

	/* storage identity changes under us -> binding breaks -> UNVERIFIED (DoD#3) */
	strlcpy(g_storage_uuid, "ffffffffffffffffffffffffffffffff",
			sizeof(g_storage_uuid));
	UT_ASSERT_EQ(cluster_cf_contract_load(pgdata), CLUSTER_CF_CONTRACT_UNVERIFIED);
