...
UT_TEST(test_external_fence_plan_shape_exact)
{
	...
	UT_ASSERT(cluster_recovery_fence_plan_shape_valid(1, replay, foreign, origins, 2));
...
UT_TEST(test_external_fence_plan_native_and_restore_gate)
{
	...
	UT_ASSERT(cluster_recovery_fence_plan_shape_valid(3, bitmap, (uint64[2]){ 0, 0 }, NULL, 0));
	UT_ASSERT(!cluster_recovery_restore_has_foreign(bitmap, 3));
	bitmap[0] |= UINT64_C(1) << 6;
	UT_ASSERT(cluster_recovery_restore_has_foreign(bitmap, 3));
	UT_ASSERT(cluster_recovery_restore_first_foreign(bitmap, 3, &foreign_thread));
