 ...
 *	  binary can verify:
 *
 *	  - CLUSTER_WAIT_EVENTS_COUNT == 46 (matches spec-0.11 registration).
...
UT_TEST(test_cluster_wait_events_count_is_56)
{
	/*
	 * Anchored against the cumulative registration roster: 46 events
	 * from spec-0.11 plus 5 added by spec-1.1 for cluster_shared_fs
	 * plus 5 added by spec-1.10 for the postmaster startup phase
	 * machinery.  If a future subsystem spec adds new cluster wait
	 ...
	 * in lockstep.
	 */
	UT_ASSERT_EQ(CLUSTER_WAIT_EVENTS_COUNT, 56);
}
...
{
	UT_PLAN(4);
	UT_RUN(test_cluster_wait_events_count_is_56);
