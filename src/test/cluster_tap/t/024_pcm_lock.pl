...
is($node->safe_psql(
   ...
   '37',
   'L7b pg_cluster_state has 37 distinct categories (spec-5.4 adds sequence)');
...
my $smoke_categories = $node->safe_psql(
	'postgres',
	q{SELECT count(DISTINCT category) FROM pg_cluster_state});
is($smoke_categories, '37', 'L9 cluster_smoke surface integrates pcm + gcs + tt_status + tt_status_hint + tt_2pc + tt_recovery + undo_record + visibility + wal_thread categories (37 categories;spec-5.4 adds sequence)');
