...
is( $node->safe_psql('postgres',
	...
	'82',
	'pg_stat_cluster_injections returns 82 rows (compile-time registry; +2 spec-1.18 WAL emit + replay observe)');
...
is( $node->safe_psql(
	...
	'82 injection point names match spec-0.30 + ... + spec-1.18 (2 cluster-scn-{wal-write,replay-observe}-pre points)');
