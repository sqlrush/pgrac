...
is( $node->safe_psql('postgres',
	...
	'125',
	'pg_stat_cluster_injections returns 125 rows (spec-4.11 +1 cluster-thread-recovery-drive)');
...
is( $node->safe_psql(
	...
	'125 injection point names match the full registry (spec-4.11 +1 cluster-thread-recovery-drive)');
