...
is( $node->safe_psql('postgres',
	...
	'127',
	'pg_stat_cluster_injections returns 127 rows (spec-4.8ab +2 undo boundary guards)');
...
is( $node->safe_psql(
	...
	'127 injection point names match the full registry (spec-4.8ab +2 undo boundary guards)');
