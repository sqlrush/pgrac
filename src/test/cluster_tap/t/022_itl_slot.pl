...
is($node->safe_psql(
   ...
   '114',
   'L12a pg_stat_cluster_injections is 114 after spec-3.1');

is($node->safe_psql(
		'postgres',
		'SELECT count(*) FROM pg_stat_cluster_wait_events'),
   '91',
   'L12b pg_stat_cluster_wait_events returns 91 rows after spec-2.39');

is($node->safe_psql(
		'postgres',
		'SELECT count(*) FROM pg_cluster_shmem'),
	   '37',
	   'L12c pg_cluster_shmem returns 37 rows (spec-3.4b D3 adds tt slot allocator region)');
