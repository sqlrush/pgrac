...
is($node->safe_psql(
   ...
   '124',
   'L15 total injection registry size is 124 (spec-4.6 +1 cluster-grd-redeclare-skip)');
...
is($node->safe_psql(
	   ...
	   '98',
	   'L17 pg_stat_cluster_wait_events returns 98 rows (spec-4.6 +1 GRD shard remaster)');
