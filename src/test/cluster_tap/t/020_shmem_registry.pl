...
is($node->safe_psql(
   ...
   '139',
   'L15 total injection registry size is 139 (spec-5.13 +6 cluster-clean-leave-*; spec-5.2a +1 clean-xfer stale-holder; spec-4.8ab +2 undo boundary guards; spec-5.7 +1 cluster-ko-peer-skip-ack; spec-2.41 +1 cluster-gcs-block-stale-ship; spec-5.55 Hardening v1.1 +1 cluster-cr-resolver-memo-suspect)');
...
is($node->safe_psql(
	   ...
	   '102',
	   'L17 pg_stat_cluster_wait_events returns 100 rows (spec-4.6 +1 GRD shard remaster)');
