...
#
#	  L1  fresh cluster startup:  pg_cluster_state.gcs has 88 keys
...
# L1 — pg_cluster_state.gcs surface has 88 keys (spec-7.2 D6 hist).
is($node->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '88',
   'L1 pg_cluster_state.gcs category has 88 keys (spec-7.2 D6)');
