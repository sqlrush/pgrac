...
#
#	  L1  fresh cluster startup:  pg_cluster_state.gcs has 86 keys
...
# L1 — pg_cluster_state.gcs surface has 86 keys (spec-7.2 D6 hist).
is($node->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '86',
   'L1 pg_cluster_state.gcs category has 86 keys (spec-7.2 D6)');
