...
#	  L9  pg_cluster_state.gcs category has 86 keys (spec-7.2 D6+flip) (cumulative through spec-6.14a)
...
is($pair->node0->safe_psql('postgres',
	...
	'120',
	'L2 pg_stat_cluster_wait_events returns 120 rows (spec-6.13 RDMA wait surface)');

is($pair->node0->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '86',
   'L2 pg_cluster_state.gcs category has 86 keys (spec-7.2 D6+flip) (cumulative through spec-6.14a)');
...
# ============================================================
# L9 (alias of L2): gcs key count = 86.
# ============================================================
is($pair->node1->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '86',
   'L9 node1 pg_cluster_state.gcs has 86 keys (cross-node parity)');
