...
#	  L9  pg_cluster_state.gcs category has 56 keys (spec-2.36 44 + 4)
...
is($pair->node0->safe_psql(
   ...
   '56',
   'L2 pg_cluster_state.gcs category has 56 keys (spec-2.36 44 + spec-2.37 4)');
...
is($pair->node1->safe_psql(
		...
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '56',
   'L9 node1 pg_cluster_state.gcs has 56 keys (cross-node parity)');
