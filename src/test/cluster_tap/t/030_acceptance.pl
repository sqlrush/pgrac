...
is($node->safe_psql('postgres',
	...
	'139', 'M1 139 injection points (spec-5.13 +6 cluster-clean-leave-*; spec-2.41 +1 gcs-block-stale-ship; spec-5.7 D6 +1 ko-peer-skip-ack; spec-5.2a +1 clean-xfer stale-holder; spec-4.8ab +2 undo boundary guards; spec-5.55 Hardening v1.1 +1 cluster-cr-resolver-memo-suspect)');
...
ok( $node->safe_psql(
	...
	'M5 inject category has 139×2 = 278 sub-keys (.fault_type + .hits; spec-5.13 +6 cluster-clean-leave-*; spec-2.41 +1 gcs-block-stale-ship; spec-5.55 Hardening v1.1 +1 cluster-cr-resolver-memo-suspect)');
...
is($node->safe_psql('postgres',
	...
	'O2 pg_cluster_state has all 48 categories (spec-5.11 adds hang; spec-5.51 adds cr_pool; spec-5.14 adds reconfig_touched; spec-5.55 adds resolver_cache)');
