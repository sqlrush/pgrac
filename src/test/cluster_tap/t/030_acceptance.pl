...
is($node->safe_psql('postgres',
	...
	'139', 'M1 139 injection points (spec-5.13 +6 cluster-clean-leave-*; spec-5.13 Hardening v1.0.3 +1 clean-leave-survivor-suppress-preflight-ack; spec-2.41 +1 gcs-block-stale-ship; spec-5.7 D6 +1 ko-peer-skip-ack; spec-5.2a +1 clean-xfer stale-holder; spec-4.8ab +2 undo boundary guards)');
...
ok( $node->safe_psql(
	...
	'M5 inject category has 139×2 = 278 sub-keys (.fault_type + .hits; spec-5.13 +6 cluster-clean-leave-*; spec-2.41 +1 gcs-block-stale-ship)');
...
is($node->safe_psql('postgres',
	...
	'advisory,block_format,buffer_format,cf,cluster_cssd,cluster_stats,conf,cr,cr_pool,diag,dl,gcs,gcs_recovery,ges,grd,grd_recovery,guc,hang,hw,ic,inject,ir,ko,lck,lmd,lmon,lms,pcm,pgstat,phase,reconfig_join,reconfig_touched,recovery,resolver_cache,scn,sequence,shared_fs,shmem,sinval,ts,tt_2pc,tt_recovery,tt_status,tt_status_hint,undo,undo_cleaner,visibility,wal_thread,write_fence',
	'O2 pg_cluster_state has all 49 categories (spec-5.11 adds hang; spec-5.51 adds cr_pool; spec-5.14 adds reconfig_touched; spec-5.15 adds reconfig_join; spec-5.55 adds resolver_cache)');
