...
is($node->safe_psql('postgres',
	...
	'121',
	'E1 pg_stat_cluster_wait_events returns 121 rows (spec-6.13 RDMA wait surface)');
...
is($node->safe_psql('postgres',
	...
	'121', 'E4 pg_stat_gcluster_wait_events returns 121 rows (single-node, spec-6.13 RDMA wait surface)');
