...
ok( $node->safe_psql(
	...
	'M5 inject category has 151×2 = 302 sub-keys (.fault_type + .hits; spec-6.12i +1 cluster-lms-undo-fetch; spec-6.12b +1 cluster-lms-cr-construct; spec-6.12a ㉕ +1 cluster-gcs-block-remote-downgrade; spec-5.13 +6 cluster-clean-leave-*; spec-2.41 +1 gcs-block-stale-ship; spec-5.55 Hardening v1.1 +1 cluster-cr-resolver-memo-suspect; spec-5.15 Hardening v1.1 +1 cluster-reconfig-join-commit-marker-durable)');
...
is($node->safe_psql('postgres',
	...
	'O2 pg_cluster_state has all 53 categories (spec-6.12 adds xnode_lever; spec-6.2 adds smart_fusion)');
