...
#      - It returns exactly 120 rows (1 node x 118 cluster wait events).
...
# ----------
# Total row count: 1 node x 118 events (spec-6.13 adds RDMA busypoll + inline send waits).
# ----------
is($node->safe_psql('postgres',
		'SELECT count(*) FROM pg_stat_gcluster_wait_events'),
	'120',
	'pg_stat_gcluster_wait_events returns 120 rows (spec-6.13 RDMA wait surface)');
