...
#	  L3  pg_cluster_state.gcs category has 51 keys (31 spec-2.34 + 7 spec-2.35)
...
# ============================================================
# L3: pg_cluster_state.gcs has 51 keys (31 spec-2.34 + 7 spec-2.35).
# ============================================================
is($pair->node0->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '51',
   'L3 node0 pg_cluster_state.gcs category has 51 keys');
is($pair->node1->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '51',
   'L3 node1 pg_cluster_state.gcs category has 51 keys');
