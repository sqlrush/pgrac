...
#	  L3  pg_cluster_state.gcs has 56 keys (22 spec-2.33 + 9 spec-2.34)
...
# ============================================================
# L3: pg_cluster_state.gcs category has 56 keys (22 spec-2.33 + 9 spec-2.34).
# ============================================================
is($pair->node0->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '56',
   'L3 node0 pg_cluster_state.gcs category has 56 keys');
is($pair->node1->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '56',
   'L3 node1 pg_cluster_state.gcs category has 56 keys');
