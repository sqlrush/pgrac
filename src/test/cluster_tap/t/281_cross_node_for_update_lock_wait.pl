...
my $pair = PostgreSQL::Test::ClusterPair->new_pair(
	...
	extra_conf          => [
		...
		# peer is falsely declared DEAD and the in-flight cross-node FOR UPDATE
		# fails its X transfer from the (falsely) dead holder BEFORE it can
		# block in the TX enqueue wait (observed: "peer 0 -> DEAD (elapsed
		# 3706 ms > 3000 ms)" then "could not obtain X transfer from X holder
		# 0").  Widen the misscount to 20s (2000ms x 10) so the lock-hold
		# window survives scheduling jitter.  This test exercises the wait
		# path, not CSSD death detection (covered by t/085 / the reconfig suite).
