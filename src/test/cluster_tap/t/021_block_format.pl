...
is($node->safe_psql(
   ...
   '125',
   'L11 pg_stat_cluster_injections is 125 (spec-4.11 +1 cluster-thread-recovery-drive)');
...
is($node->safe_psql(
   ...
   '99',
   'L12 pg_stat_cluster_wait_events returns 99 rows (spec-4.6 +1 GRD shard remaster)');
