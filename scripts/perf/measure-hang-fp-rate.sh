#!/bin/bash
#-------------------------------------------------------------------------
# measure-hang-fp-rate.sh -- spec-5.20 D6 (MG-A) wrapper.
#
# Sets up the --enable-cluster install on PATH + IPC::Run on PERL5LIB, then
# delegates to measure-hang-fp-rate.pl (which drives PostgreSQL::Test::Cluster).
...
# IDENTIFICATION
#    scripts/perf/measure-hang-fp-rate.sh
#-------------------------------------------------------------------------
set -euo pipefail
...
echo "measure-hang-fp-rate: install=$ENABLE_INSTALL window=${WINDOW}s ts=$TS"
perl "$REPO_ROOT/scripts/perf/measure-hang-fp-rate.pl" \
